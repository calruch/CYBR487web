...
def main():
    ...
    # No Host Discovery option
        ...
        # Traceroute TCP only
        if scanType == 4:  
            scanList = tracerouteScanner.scan(ipList, maxHops=maxHops, verbose=verbose, icmpTCPoption=2)
            if scanList is not None:
                for i in range(0, len(scanList)):
                    report.printTraceroute(scanList[i])

        # Traceroute ICMP only
        if scanType == 5:  
            scanList = tracerouteScanner.scan(ipList, maxHops=maxHops, verbose=verbose, icmpTCPoption=1)
            if scanList is not None:
                for i in range(0, len(scanList)):
                    report.printTraceroute(scanList[i])
        return
